...
|------------|------------|-----------------|
| class `BehaviourTree` <TContext, TLeafState> |  | runtime-BT, создается 1 экземпляр для каждого скомпилированного BT-ассета перед началом игры. TContext - ваш контекст BT для каждого агента. TLeafState - ваше состояние для каждого листа |
